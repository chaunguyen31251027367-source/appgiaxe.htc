...
/* Background */
.stApp {
    background: linear-gradient(135deg, #1a0a12 0%, #2d0f20 50%, #1f0a1a 100%);
}

/* Sidebar */
[data-testid="stSidebar"] {
    background: linear-gradient(180deg, #150810 0%, #2d0f20 100%);
    border-right: 1px solid rgba(255,105,180,0.25);
}
...
.hero-title {
    ...
    font-size: 3.8rem;
    letter-spacing: 4px;
    background: linear-gradient(90deg, #FF2D78, #FF85C2, #FF2D78);
    background-size: 200%;
...
/* Metric cards */
.metric-card {
    background: linear-gradient(135deg, rgba(255,45,120,0.12) 0%, rgba(255,133,194,0.06) 100%);
...
.pred-box {
    background: linear-gradient(135deg, #FF2D78 0%, #FF69B4 50%, #FF85C2 100%);
    border-radius: 20px;
    padding: 36px 24px;
    text-align: center;
    box-shadow: 0 8px 40px rgba(255,45,120,0.4);
}
.pred-label {
...
[data-testid="stMetricValue"] { color: #FF85C2 !important; font-family: 'Bebas Neue', sans-serif !important; font-size: 2rem !important; }
[data-testid="stMetricLabel"] { color: #aaaacc !important; font-size: 0.82rem !important; text-transform: uppercase; letter-spacing: 1px; }
div.stButton > button {
    background: linear-gradient(135deg, #FF2D78, #FF85C2);
    color: #0f0f1a;
    font-family: 'Bebas Neue', sans-serif;
    font-size: 1.1rem;
    letter-spacing: 2px;
...
with st.sidebar:
    ...
    <div style='text-align:center; padding: 16px 0 8px 0;'>
        ...
        <div style='font-family: Bebas Neue, sans-serif; font-size:1.5rem; letter-spacing:3px; color:#FF85C2;'>
            MyOldMotorbike
        </div>
    ...
    st.markdown("""
    <hr style='border-color:rgba(255,45,120,0.15); margin:20px 0 12px 0;'>
    <div style='font-size:0.75rem; color:#666; text-align:center; line-height:1.8;'>
        <b style='color:#FF2D78'>Dữ liệu</b>: 243 xe cũ tại TP.HCM<br>
        <b style='color:#FF2D78'>Mô hình</b>: Random Forest & Gradient Boosting<br>
        <b style='color:#FF2D78'>Năm</b>: 2024–2026
    </div>
    """, unsafe_allow_html=True)
...
PLOTLY_THEME = dict(
    ...
    title_font=dict(color='#FF85C2', family='Nunito', size=15),
    ...
    colorway=['#FF2D78','#FF85C2','#FF69B4','#FFB6C1','#F48FB1','#F06292','#EC407A'],
)
ORANGE_SEQ = px.colors.sequential.RdPu
...
if page == "📊 Tổng Quan Dữ Liệu":
    ...
    with col1:
        ...
        fig = px.histogram(df, x='Gia_ban', nbins=30,
                           ...
                           color_discrete_sequence=['#FF2D78'])
        fig.update_traces(opacity=0.85)
        fig.add_vline(x=df['Gia_ban'].mean(), line_dash='dash', line_color='#FF85C2',
                      annotation_text=f"TB: {df['Gia_ban'].mean()/1e6:.1f}M",
                      annotation_font_color='#FF85C2')
    ...
    with col3:
        ...
        fig = px.histogram(df, x='Nam_sx', nbins=20,
                           ...
                           color_discrete_sequence=['#FF85C2'])
        fig.update_layout(**PLOTLY_THEME)
        st.plotly_chart(fig, use_container_width=True)
...
# ═══════════════════════════════════════════════
    ...
    with col1:
        ...
        fig = px.box(filtered, x='Hang_xe', y='Gia_ban',
                     ...
                     color_discrete_sequence=['#FF2D78','#FF85C2','#FF69B4','#FFB6C1','#F48FB1','#F06292','#EC407A','#E91E8C'])
...
# ═══════════════════════════════════════════════
    ...
    with col1:
        fig = go.Figure(go.Bar(
            x=names, y=r2_vals,
            marker_color=['#FF85C2' if n == best_name else '#FF2D78' for n in names],
    ...
    colors_radar = ['#FF2D78','#FF85C2','#FF69B4']
    fig = go.Figure()
    ...
    fig.update_layout(
        polar=dict(
            ...
            angularaxis=dict(gridcolor='rgba(255,255,255,0.1)', color='#FF85C2'),
        ),
    ...
    with col_result:
        ...
        if predict_btn:
            ...
            fig = go.Figure(go.Indicator(
                ...
                value=percentile,
                title={'text': "Vị trí giá trong thị trường", 'font': {'color': '#FF85C2', 'family': 'Nunito'}},
                number={'suffix': "% phần trăm vị", 'font': {'color': '#FF85C2', 'family': 'Nunito'}},
                gauge={
                    ...
                    'threshold': {'line': {'color': '#FF85C2', 'width': 3}, 'value': percentile}
                }
            ))
            ...
            st.markdown("""
            <div style='text-align:center; padding:60px 20px; border:2px dashed rgba(255,45,120,0.25); border-radius:16px; color:#666;'>
                <div style='font-size:3rem'>🏍️</div>
                <div style='font-size:1rem; font-weight:600; margin-top:12px;'>
                    Nhập thông tin xe và nhấn<br><span style='color:#FF2D78'>DỰ ĐOÁN GIÁ NGAY</span>
                </div>
            </div>
            """, unsafe_allow_html=True)
    ...
    if predict_btn:
        ...
        with ca:
            fig = px.histogram(brand_data, x='Gia_ban',
                               ...
                               color_discrete_sequence=['#FF2D78'], nbins=20)
            fig.add_vline(x=predicted, line_color='#FF85C2', line_dash='dash',
                          annotation_text="Xe bạn", annotation_font_color='#FF85C2')
            fig.update_layout(**PLOTLY_THEME)
            st.plotly_chart(fig, use_container_width=True)
        ...
        with cb:
            if len(model_data) > 3:
                ...
                fig.add_hline(y=predicted, line_color='#FF85C2', line_dash='dash',
                              annotation_text="Dự đoán", annotation_font_color='#FF85C2')
                fig.update_layout(**PLOTLY_THEME)
                st.plotly_chart(fig, use_container_width=True)
            else:
...
# ─────────────────────────────────────────────
st.markdown("""
<hr style='border-color:rgba(255,45,120,0.15); margin:40px 0 16px 0;'>
<div style='text-align:center; color:#555; font-size:0.8rem; padding-bottom:20px;'>
    🏍️ <b style='color:#FF2D78'>MyOldMotorbike</b> · Dự án Machine Learning · TP. Hồ Chí Minh · 2026<br>
